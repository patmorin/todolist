...
$\eps=0.35$, insertions take three to four times as long in
...
$B$-trees \cite{bayer.mccreight:organization}
and their cache-oblivious counterparts
\cite{bender.demaine.ea:cache-oblivious,bender.duan.ea:locality-preserving}
can reduce the number of cache misses to $O(\log_B n)$ per search,
where $B$ is the size of a cache line.
...
Our working todo-list shows that it is possible to obtain a data structure
with $O(\log w(x))$ search time that performs only $(1+\eps)\log w(x) + o(\log w(x))$
comparisons per search, for any constant $\eps > 0$.  This leaves
