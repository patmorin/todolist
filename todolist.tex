...
constants.\footnote{Indeed, red-black trees were
...
%\addtolength{\pageheight}{1in}
Finally, we now undestand why, despite
being simple to implement and theoretically fast, skiplists are not
used much in practice: Their standard implementation
results in too many cache misses.  
Our implementation trick
of storing the next key value along with each pointer can be used to 
make skiplists competitive again.
...
The experiments described in this section were done with 128 bit
integer keys.  These experiments are included mainly in response to
a referee who felt that the caching behaviour that makes todolists so
fast might be negated if the key length were increased.  The results in
this appendix show that this is not the case.  Todolists continue to be
faster at searching than other data structures.
