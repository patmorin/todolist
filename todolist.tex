...
``second value'' as needed to ensure that Property~3 holds. 
...
$O(\sqrt{\log w(x)})$ term accounts for one comparison at each of the
lists $L_{\lceil\log_{2-\eps} w(x)\rceil},\ldots,L_i$ as well as the
extra comparison (equality test) performed in each of the lists $L_j$
where $j\in\{0,\ldots,i\}$ is a perfect square.
...
the experimental data here was obtained from the program
\texttt{main.cpp}, which was
compiled using the command line: \texttt{g++ -std=c++11 -Wall -O4 -o main main.cpp}. The compiler was the \texttt{gcc} compiler, version
...
Figures~\ref{fig:epsilon} and \ref{fig:epsilon-128} show the
results of varying the value of $\eps$ from $0.02$ to $0.68$ in
increments of $0.01$. In this figure, $n=10^6$ random integers in
the set $\{0, 5,\ldots,5(n-1)\}$ were chosen (with replacement) and
inserted into a todolist. Since dictionaries discard duplicates, the
resulting todolist contained $906,086$ values.  This todolist was then
searched $m=5n$ times with random integers chosen, with replacement,
from $\{-2,\ldots,5n+3\}$.  This figure illustrates that todolists do
behave roughly as \thmref{todolist} predicts.  Insertion time increases
roughly proportionally to $1/\eps$ and search times seem to be of the
form $c(d+\eps)$ for some constant $c$ and $d$ (though there is certainly
lots of noise in the search times).
...
The timing results for the searches are shown in
Figures~\ref{fig:bigsearch} and \ref{fig:bigsearch-128}. In terms
of search times, todolists---with $\eps = 0.2$ and $\eps=0.35$---are
...
for $\alpha\in[1.02,1.03]$.\footnote{The remarkable performance
...
that the average depth of a node in this case is $\log
...
Figures~\ref{fig:biginsertion} and \ref{fig:biginsertion-128}. This
is where the other shoe drops.  Even with $\eps=0.35$, insertions
take three to four times as long in a todolist as in a red-black tree.
Profiling the code shows that approximately 65\% of this time is spent
doing partial rebuilding and another 6\% is due to global rebuilding.
...
$\frac{1}{2}\log_{2-\eps} n$ cache misses on average.   
...
Our experimental results also explain why, despite being simple
to implement and theoretically fast, skiplists are not used much in
practice: Their standard implementation results in too many cache misses.
Our implementation trick of storing the next key value along with each
pointer can be used to make skiplists competitive again.

One avenue for future experimental work in
this area would be to compare the data structures
presented here to efficient implementations of $B$-trees
...
which can reduce the number of cache misses to $O(\log_B n)$ per
search, where $B$ is the size of a cache line.  We are unaware of any
implementations of cache-oblivious search treees that are competitive
in RAM.  However, preliminary testing of the Google B-tree C++ containers
suggests that they outperform the usual STL set implementation by a
significant margin \cite{macdonald:containers}.

%\footnote{Indeed, red-black trees were
%originally designed as a simpler alternative to 2--4-trees and red-black trees
%are considerably more difficult to implement than todolists. (This
%is the subjective opinion of the second author, but most people who have
%implemented red-black trees from scratch would agree.)}
On a more theoretical note, our working todo-list is a data structure
with, for any constant $\eps>0$, $O(\log w(x))$ search time that performs only $(1+\eps)\log w(x)
+ o(\log w(x))$ comparisons per search.
This leaves a tantalizing open problem:

\begin{op}
   Is there a comparison-based data structure that has $O(\log w(x))$
   search time and that performs $\log w(x)+o(\log w(x))$ comparisons
   per search?
\end{op}
...
\subsection{Experiments with 32 bit keys}

The experiments described in this section were done with 32 bit integer keys.

\subsubsection{Adjusting the value of $\eps$ (32 bit keys)}
...
\subsubsection{Search times (32 bit keys)}
...
\subsubsection{Insertion times (32 bit keys)}
...
\subsection{Experiments with 128 bit keys}

The experiments described in this section were done with 128 bit integer
keys.  These experiments are included mainly to exclude the possibility
that the caching behaviour that makes todolists so fast might be negated
if the key length were increased.  The results in this appendix show that
this is not the case.  Todolists continue to be faster at searching than
other data structures.

\subsubsection{Adjusting the value of $\eps$ (128 bit keys)}

\begin{figure}[H]
  \centering{\input{experiments-wide/epsilon.tex}}
  \caption{The tradeoff between search time and insertion time as a function of $\eps$.}
   \figlabel{epsilon-128}
\end{figure}

\subsubsection{Search times (128 bit keys)}

\begin{figure}[H]
  %\resizebox{\tmpa}{!}{\input{images/bigoh-1.tex}}\\[4ex]
  \centering{\input{experiments-wide/bigtest-find.tex}}
  \centering{\input{experiments-wide/bigtest-find-norm.tex}}
  \caption{Search time comparison between different data structures. The top graph shows absolute times, in seconds. The bottom graph shows relative times, normalized by the time taken in the optimal search tree.}
   \figlabel{bigsearch-128}
\end{figure}

\subsubsection{Insertion times (128 bit keys)}

\begin{figure}[H]
  %\resizebox{\tmpa}{!}{\input{images/bigoh-1.tex}}\\[4ex]
  \centering{\input{experiments-wide/bigtest-add.tex}}
  \centering{\input{experiments-wide/bigtest-add-norm.tex}}
  \caption{Insertion time comparison between different data structures. The top graph shows absolute times, in seconds. The bottom graph shows relative times, normalized by the running time of the red-black tree.}
   \figlabel{biginsertion-128}
\end{figure}
